...
class HourglassNet3D(object):
	...
	def __init__(self, nChannels, nStack = 2, nModules = 1, numReductions = 3, nJoints = 25):
		...
		for _ in range(self.nStack):
			...
			_chantojoints.append(nn.Conv3d(self.nChannels, self.nJoints))
			_lin2.append(nn.Conv3d(self.nChannels, self.nChannels))
			_jointstochan.append(nn.Conv3d(self.nJoints,self.nChannels))
