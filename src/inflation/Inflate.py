...
def inflateconv(conv3d, conv):
	...
	conv3d.W= conv.weight.data[:,:,None,:,:].expand_as(conv3d.W).clone().contiguous()
	conv3d.B = conv.bias.data.contiguous()
	conv3d.W = torch.nn.Parameter(conv3d.W)
	conv3d.B = torch.nn.Parameter(conv3d.B)
	return
