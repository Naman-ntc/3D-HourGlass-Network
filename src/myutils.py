...
def give3D(output2D,output3D,meta):
	out = torch.cat(softGetPreds(output2D), output3D)
	out[:,:,:,2] = (out[:,:,:,2] + 1) / 2 * ref.outputRes
	h36mSumLen = 4296.99233013
	root = 6
	lens = torch.zeros_like(out[:,0,:,0]).cuda()
	for e in ref.edges:
		lens += torch.norm(out[:,e[0],:,:] - out[:,e[1],:,:], dim=-1)
	rootRelative = h36mSumLen * ((out[:,:,:,:] - out[:,root:root+1,:,:]) / lens) + meta[:,root,:,:]

	rootRelative[:,7,:,:] = rootRelative[:,8,:,:]/2
	return rootRelative
